...
| ISS-003 | RQ-DAT-002, RQ-NFR-001 | App crashes on Android at startup: `dlopen failed: library "libsqlcipher.so" not found` | `sqlcipher_flutter_libs: ^0.7.0` in `pubspec.yaml` resolved to `0.7.0+eol`, a tombstone package with an empty Dart file and **no native libraries**. `libsqlcipher.so` was never bundled in the APK. Secondary issue: `open.overrideFor()` must be called inside the background Dart isolate spawned by `NativeDatabase.createInBackground` (each isolate has independent global state) via `isolateSetup`. | Downgraded `sqlcipher_flutter_libs` to `^0.6.0` (resolves to `0.6.8`) which bundles `libsqlcipher.so` for `arm64-v8a`, `armeabi-v7a`, and `x86_64`. Added top-level `openSqlcipherLibrary()` (bare dlopen + full-path fallback via `/proc/self/cmdline`) passed via `_overrideSqliteLibraryForAndroid()` as `isolateSetup` to `NativeDatabase.createInBackground`. APK grew from ~25 MB to 69.6 MB confirming the `.so` is now bundled. Added warning comment in `pubspec.yaml` to prevent re-upgrade to v0.7. Two passing unit tests in `test/data/database/database_factory_test.dart`. See ADR-015. | 2026-04-01 |
